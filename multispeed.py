...
def gen_ffmpeg_cmd(args) -> str:
    ...
    if ia > 0:
        ...
        map += " -map [out_a]"
    command = "ffmpeg -i \"{}\" -filter_complex \"{}\" {} -c:v h264 -c:a aac \"{}\"".format(input, filter_complex, map,
                                                                                            output)
    return command


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Help write trimming video code in ffmpeg command by giving text file.")
    parser.add_argument("-file", "-f", help="input text file, including trimming time.", type=str, default="trim.txt")
    parser.add_argument("-text", "-t", help="input text, including trimming time.", type=str, default="")
